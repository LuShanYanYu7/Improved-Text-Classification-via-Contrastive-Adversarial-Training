...
# lr = [0.00001, 0.00002, 0.00003, 0.00004, 0.00005]
...
optimizer = AdamW(list(model.parameters()) + list(classifier.parameters()), lr=lr, weight_decay=0.01)
...
for epoch in range(epochs):
    ...
    for i, batch in enumerate(train_loader):
        ...
        # 计算总损失
        total_batch_loss = (1 - lambda_) * (original_loss + perturbed_loss) / 2 + mlp_similarity_loss * lambda_
